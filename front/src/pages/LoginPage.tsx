...
export default function LoginPage() {
  ...
  const handleVerify2fa = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await api.post('/auth/verify-2fa', { temp_token: tempToken, code: twoFaCode })
      storeSession(res.data.data)
      navigate('/dashboard', { replace: true })
    } catch (err: any) {
        ...
        {twoFaStep ? (
          <form className="space-y-6" onSubmit={handleVerify2fa}>
            <div className="text-center">
              ...
              <h2 className="text-headline-md text-on-surface">Vérification en 2 étapes</h2>
            ...
            <div className="space-y-2">
              <label className="block text-label-lg text-on-surface" htmlFor="code">
              ...
              <input
                ...
                placeholder="• • • • • •"
                className="w-full px-4 py-4 bg-white border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-center text-2xl font-mono tracking-[0.5em] outline-none"
                required
                autoFocus
              />
            </div>
            ...
            <button
              type="button"
              onClick={() => { setTwoFaStep(false); setTwoFaCode(''); setTempToken(''); setError('') }}
          ...
          <>
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              <div className="space-y-2">
                ...
                <input
                  ...
                  id="email" type="email" placeholder="nom@votreclub.fr"
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <label className="block text-label-lg text-on-surface" htmlFor="password">Mot de passe</label>
                  <Link to="/forgot-password" className="text-label-md text-primary hover:underline">Oublié ?</Link>
                </div>
            ...
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-label-md"><span className="px-3 bg-white text-on-surface-variant">ou</span></div>
            </div>
            ...
            {isDev && (
              <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                ...
                <p className="text-body-sm text-amber-700 mb-2">Base de données non disponible ? Utilisez le mode démo.</p>
            ...
            <footer className="mt-6 text-center space-y-3">
              <p className="text-body-md text-on-surface-variant">
                Pas de compte ?{' '}
                <Link to="/register" className="text-primary text-label-lg hover:underline decoration-2 underline-offset-4 ml-1">S'inscrire</Link>
              </p>
              <div className="relative">
                <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-outline-variant" /></div>
                <div className="relative flex justify-center"><span className="px-3 bg-white text-label-md text-on-surface-variant">ou</span></div>
              </div>
              <Link to="/resultats-club" className="flex items-center justify-center gap-2 text-on-surface-variant hover:text-primary text-label-md transition-colors">
