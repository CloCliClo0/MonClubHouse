...
* **ajouter, modifier ou supprimer des tests unitaires si manquants ou obsolètes.**
...
* `.env`
* `.env.example`
* `.gitignore` (vérifie que les fichiers de mémoire de la discussion soient bien dedans (ex: `./changelog`, `./context`, `./memory`, `instruction.md`, `./test-unitaire` etc.))
* fichiers `.sql`
...
### Gestion et Stratégie des Tests
Mettre à jour : `./test-unitaire/test-global.md` (si l'organisation globale des tests, les profils ou les formats de tests évoluent).
...
* **Lancer la suite de tests unitaires par profil pour s'assurer de l'absence de régressions.**
...
* Dit moi à chaque fin de réflexion et travail, s'il est nécessaire de pousser à nouveau les fichiers .env et .sql sur le serveur en cas de modification.